...
$Users = Get-ADUser -Server "tb-dc-062" -filter {Enabled -eq $True -and PasswordNeverExpires -eq $False} -Properties msDS-UserPasswordExpiryTimeComputed, PasswordLastSet, CannotChangePassword, mail, GivenName
$date = (Get-Date).AddDays(5)
foreach ($user in $users){
    $expire = $user | Select-Object Name, @{Name="ExpiryDate";Expression={[datetime]::FromFileTime($_."msDS-UserPasswordExpiryTimeComputed")}}, PasswordLastSet, mail, GivenName
    ...
    if($($expire.ExpiryDate) -ge (Get-Date) -and $($expire.ExpiryDate) -le $date){
        ...
        Write-Host "$($expire.GivenName) - $($expire.ExpiryDate.ToShortDateString()) - $($expire.mail)"
        ...
        if ($($vcto.Days) -eq "0"){
            ...
            $body += "Para trocar a senha, utilize o Portal de Senhas, clicando em <a href='https://aka.ms/sspr'>https://aka.ms/sspr</a>. Importante encerrar o acesso a VPN caso esteja conectado(a).<br><br>"
            $body += "Quando você utiliza o computador sem estar conectado na VPN, a autenticação (digitação do login e senha) ocorre em cache, ou seja, o Windows utiliza a senha que está armazenada local e não a nova senha cadastrada no portal.<br><br>"
            $body += "Depois de realizar a troca da senha no portal, será necessário se autenticar na VPN utilizando a nova senha, em seguida pressione as teclas CTRL + ALT + DEL para fazer o bloqueio do Windows/sessão, em seguida faça o desbloqueio digitando a nova senha cadastrada no portal.<br><br>"
